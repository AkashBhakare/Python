# -*- coding: utf-8 -*-
"""
Created on Wed Nov 17 08:33:19 2021
...
def copyTextFile(srcFile:str,destFile:str):
    ...
    if not os.path.isfile(srcFilePath):
        print(f'{srcFile} does not exists')
        sys.exit(0)
    ...
    infile  = open(srcFilePath,  mode  = "r")
    outfile = open(destFilePath, mode = "w")
...
def appendTextFile(srcFile,destFile):
    ...
    if not os.path.isfile(destFilePath):
        print(f"file {destFile} does not exists")
        sys.exit(0)
...
def copyImageFile(srcImage,destImage):
    try:
        with open(srcImage, "rb") as srcFile, open(destImage, "wb") as destFile:
            while True:
                byte = srcFile.read(1)
                # print(byte)
                if not byte:
